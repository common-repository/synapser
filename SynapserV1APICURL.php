...
define("SYNAPSER_API_VERSION", "v1_1.2.0_CURL");

/**
 * This is the Synapser API client for CURL Libs
 * 
 * @author Synapser.net
 *
 */
class SynapserV1APICURL{
...
';
	 ...
	 */
	public function SynapserV1APICURL($hashKey, $domainKey, $debugEnabled=false){
	...
	private function getURL($url){
		if($this->debugEnabled){
			echo "<!-- SynapserV1APICURL.DEBUG - getURL::url=" . $url . " --> \n";
		}
	...
	public function getCurrentPageMeta(){
		...
		if($this->debugEnabled){
			echo "<!-- SynapserV1APICURL.DEBUG - getCurrentPageMeta::pageUrl=" . $pageUrl . " --> \n";
		}
	...
	public function getPageMeta($pageUrl){
		$url = $this->meta_api_url . "?hashkey=" . $this->hashKey . "&url=" . $pageUrl;
		
		$meta = $this->getURL($url);
		
		if($this->debugEnabled){
			echo "<!-- SynapserV1APICURL.DEBUG - getPageMeta::meta= \n";
			print_r( $meta );
			echo "-->\n";
		}
	...
	public function getCurrentPageKeys(){
		...
		if($this->debugEnabled){
			echo "<!-- SynapserV1APICURL.DEBUG - getCurrentPageKeys::pageUrl=" + $pageUrl + " --> \n";
		}
	...
	public function getPageKeys($pageUrl){
		$url = $this->keys_api_url . "?hashkey=" . $this->hashKey . "&url=" . $pageUrl;

		$keys = $this->getURL($url);
		
		if($this->debugEnabled){
			echo "<!-- SynapserV1APICURL.DEBUG - getPageKeys::keys= \n";
			print_r( $keys );
			echo "-->\n";
		}
